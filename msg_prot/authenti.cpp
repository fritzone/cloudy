...
std::string Authenticate::serialize() const
{
    ...
    result += stringify(user_name_hash);
    result += "</user_name_hash>";
    // attribute:password_hash
    result += "<password_hash>";
    result += stringify(password_hash);
    result += "</password_hash>";
    result += "</attributes>";
    result += "</o>";
    return result;
...
int Authenticate::deserialize(const char* xml)
{
    ...
    destringify(user_name_hash, attr_node_user_name_hash->txt);
    ezxml_t attr_node_password_hash = ezxml_child(attrs_node, "password_hash");
    destringify(password_hash, attr_node_password_hash->txt);
...
bool Authenticate::operator == (const Authenticate& rhs) const
{
    ...
    if(user_name_hash != rhs.user_name_hash) return false;
    if(password_hash != rhs.password_hash) return false;

    return true;
}
void Authenticate::set_user_name_hash(const std::string& p_user_name_hash)
{
    user_name_hash = p_user_name_hash;
}
